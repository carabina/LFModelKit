...
//
//  Created by guoyaoyuan on 15/5/10.
//
 ...
 * Implementations of `NSCoding`, `NSCopying`, `-hash` and `-isEqual:`.
 ...
 See `LFModel` protocol for custom methods.
...
//Code
@interface NSObject (LFModelAdditions)
 ...
 */
@interface NSDictionary (LFModel)
...
/**
 ...
 There's no need to add '<LFModel>' to your class header.
 */
@protocol LFModel <NSObject>
@optional
/*
 Custom property mapper. 仅需要把不同名的属性写出来就行，同名的没必要写。
